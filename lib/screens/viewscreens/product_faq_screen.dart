...
class _ProductFAQScreenState extends State<ProductFAQScreen> {
  ...
  Future<void> _showFAQDialog({FAQModel? faqToEdit}) async {
    final questionController = TextEditingController(text: faqToEdit?.question ?? '');
    final answerController = TextEditingController(text: faqToEdit?.answer ?? '');
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (faqToEdit != null) {
                await FAQService.updateFAQ(faqToEdit.id, FAQModel(id: faqToEdit.id, question: question, answer: answer));
              } else {
                await FAQService.createFAQ(FAQModel(id: '', question: question, answer: answer));
              }
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        trailing: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            IconButton(icon: const Icon(Icons.edit, color: Colors.blue), onPressed: () => _showFAQDialog(faqToEdit: faq)),
                            IconButton(icon: const Icon(Icons.delete, color: Colors.red), onPressed: () => _deleteFAQ(faq)),
                          ],
                        ),
                      ),
                    );
